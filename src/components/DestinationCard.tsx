...
function formatMin(m: number) {
  ...
  return h > 0 ? `${h}時間${mn > 0 ? mn + "分" : ""}` : `${mn}分`;
}
...
export default function DestinationCard({ d, pp, index, queryString = "" }: { d: any, pp: boolean, index: number, queryString?: string }) {
  return (
    <Link 
      href={`/destination/${d.id}${queryString ? `?${queryString}` : ''}`}
      ...
      <div className="p-4 flex-1 flex flex-col justify-between min-w-0 bg-white relative">
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 mt-3 pr-20 md:pr-0">
            {d.themes.map((t: string) => <span key={t} className="text-[10px] bg-orange-50 text-orange-700 border border-orange-100 px-2 py-0.5 rounded-md font-bold">{t}</span>)}
            {d.scene?.slice(0, 2).map((s: string) => <span key={s} className="text-[10px] bg-blue-50 text-blue-600 border border-blue-100 px-2 py-0.5 rounded-md font-bold">{s}</span>)}
          </div>
        </div>
